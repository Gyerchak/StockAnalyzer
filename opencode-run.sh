...
nohup bash "$BOX_DIR/script/auto-handoff.sh" --watch --name "StockAnalyzer" --data-dir "$DATA" --handoffs "$HANDOFFS_DIR" --pidfile "$DATA/opencode.pid" --workdir "$PROJECTS_DIR/$PROJ" --restart-cmd "$0 launch continue" >/dev/null 2>&1 &
