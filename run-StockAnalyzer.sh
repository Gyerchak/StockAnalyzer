#!/usr/bin/env bash
# Open "StockAnalyzer" with its own dedicated sessions in a NEW TERMINAL WINDOW.
...
DATA="$PROJ_DATA/StockAnalyzer/.opencode-data"
SESSIONS_DIR="$PROJ_DATA/StockAnalyzer/sessions"
HANDOFFS_DIR="$PROJ_DATA/StockAnalyzer/handoffs"
LOCKFILE="/tmp/opencode-StockAnalyzer.lock"

export OPENCODE_CONFIG="$PROJ_DATA/StockAnalyzer/opencode.json"
...
if [ "${1:-launch}" = "launch" ]; then
  ...
  if [ -f "$LOCKFILE" ]; then
    ...
    if [ -n "$LOCK_PID" ] && kill -0 "$LOCK_PID" 2>/dev/null && [ "${2:-}" != "continue" ]; then
      echo "StockAnalyzer is already running (PID $LOCK_PID)."
...
nohup bash "$BOX_DIR/scripts/auto-handoff.sh" --watch --name "StockAnalyzer" --data-dir "$DATA" --handoffs "$HANDOFFS_DIR" --pidfile "$DATA/opencode.pid" --workdir "$PROJECTS_DIR/$PROJ" --restart-cmd "$0 launch continue" >/dev/null 2>&1 &
